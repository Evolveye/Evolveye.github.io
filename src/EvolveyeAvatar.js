...
export default class EvolveyeAvatar extends React.Component {
  ...
  refCallback = async ref => {
    ...
    const pupilWidth = pupilRX * 2
    const pupilHeight = pupilRY * 2

    const reshape = () => path.setAttribute( `d`, EvolveyeAvatar.shapeModifier( factor, true ) )
    ...
    pupil.setAttribute( `cx`, pupilCX )
    pupil.setAttribute( `cy`, pupilCY )
    pupil.setAttribute( `rx`, pupilRX )
    pupil.setAttribute( `ry`, pupilRY )

    this.setState( { animationInterval:setInterval( reshape, 1000 * this.state.interval ) } )
  ...
  render = () => <svg
    ...
    >
    <path d={this.state.path} style={{ transition:`${this.state.interval}s` }} fill="#22d"/>
